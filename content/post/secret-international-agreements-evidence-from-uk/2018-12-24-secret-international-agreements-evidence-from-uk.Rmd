...
```{r}
wd %>%
    filter(!is.na(treaty_type) & nchar(treaty_type) > 1) %>%
    distinct(document_id, .keep_all = TRUE) %>%
    mutate(year = year(date_of_signature),
...
On this last point, we find in our data politically sensitive agreements such as agreements on war trade with non-allies during the second world war, such as the agreement concluded with Portugal on 1942, and weapons and intelligence sharing agreements.


To get the general idea of the content of these agreements we can tokenize their title, remove all stop words, and compute the **inverse document frequency** ($\text{IDF}_{word_i} = \ln(\frac{n_{\text{subject-area}}}{n_{\text{subject-area with word}_{i}}})$) of each term, by subject, to find the more idosyncratic terms of each subject-area.
...
tidytext_subject <- tidy_pub %>%
  unnest_tokens(., word, title) %>%
  anti_join(stop_words, by = "word") %>%
  filter(str_detect(word, "[0-9]+|[[:punct:]]") == FALSE) %>%
  group_by(word) %>%
  mutate(n = n()) %>%
  ungroup() %>%
  bind_tf_idf(word, subject, n) %>%
  arrange(desc(tf_idf)) %>%
  filter(subject %in% c("TRADE", "COMPENSATION", "DEFENCE", "LOANS, CREDITS & GRANTS") & publication_record == "Not Published") 
...
tidytext_subject %>%
  ...
  top_n(14, wt = idf) %>%
  ungroup() %>%
  ggplot(aes(reorder(factor(word), +idf), idf, fill = subject)) +
  geom_bar(stat = "identity") + 
  coord_flip() +
  facet_wrap(~subject, scales = "free") +
  guides(fill = FALSE) +
  labs(x = "top 10 words") +
  ylab("Inverse Document Frequency")
```
